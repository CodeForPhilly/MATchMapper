...
df = pd.read_csv(os.path.join(path, 'data', 'providers_with-unpublic-data_for-scripts_574names.csv'), index_col=False)
# npi_data = df['npi'].values.tolist()
first_name_data = df['name_first'].values.tolist()
last_name_data = df['name_last'].values.tolist()
...
for i in range(len(first_name_data)):
    ...
    counter +=1
    print(counter)
    url = 'https://npiregistry.cms.hhs.gov/api/?number=&enumeration_type=&taxonomy_description=&first_name={0}&use_first_name_alias=&last_name={1}&organization_name=&address_purpose=&city=Ph*&state=PA&postal_code=19*&country_code=US&limit=&skip=&version=2.1'.format(first_name_data[i], last_name_data[i])
    response = requests.get(url)
    json_data = json.loads(response.text)
    if json_data['result_count'] == 1:
        ...
        if result.get('practiceLocations') is not None:
            ...
            for location in result['practiceLocations']:
                ...
                loc_counter += 1
        loc_counter = 0
    elif json_data['result_count'] == 0:
        pass
    else:
        print("Can't find unique match! :(")
    npi_final_data.append(npi_dict)
final_df = pd.DataFrame(npi_final_data)
final_df['download_initial'] = now
final_df['download_latest'] = now
numnames = len(final_df)
# df.columns = ['provider_id']
# df = df.reset_index(drop=True)
# final_df = pd.concat([df, final_df], axis=1)
final_df.to_csv(os.path.join(path, 'data', 'NPI_info_' + now + '_' + numnames + '.csv'), index=False)
